...
export function extractSlotIdempotencyKey(body: unknown): string | null {
...
export async function beginSlotSpinIdempotency(
  userId: string,
  idempotencyKey: string
): Promise<{ cached: Record<string, unknown> | null }> {
  const existing = await prisma.slotSpinIdempotency.findUnique({
  ...
  try {
    await prisma.slotSpinIdempotency.create({
      data: {
        userId,
        idempotencyKey,
        responseJson: "",
    ...
    if (
      ...
      const row = await prisma.slotSpinIdempotency.findUnique({
        where: { userId_idempotencyKey: { userId, idempotencyKey } },
      });
...
export async function saveSlotSpinResponse(
  userId: string,
  idempotencyKey: string,
  response: Record<string, unknown>
) {
  await prisma.slotSpinIdempotency.update({
    where: { userId_idempotencyKey: { userId, idempotencyKey } },
    data: {
...
export async function releaseSlotSpinIdempotency(
  userId: string,
  idempotencyKey: string
) {
  await prisma.slotSpinIdempotency.deleteMany({
